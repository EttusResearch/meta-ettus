...
BITFILE_NAME_ni-titanium = "usrp_x410_fpga_X4_200"

FILES_${PN}-x410 = " \
    ${UHD_IMAGES_INSTALL_PATH}/${BITFILE_NAME}.* \
    "
...
UHD_IMAGES_TO_DOWNLOAD_ni-titanium ?= " \
    ${@ '' if d.getVar('EXTERNALSRC') else 'x4xx_fpga_default'} \
    "

UHD_FPGA_IMAGES_IN_FIRMWARE_ni-titanium ?= "${BITFILE_NAME}"
...
do_download_uhd_images_append_ni-titanium() {
    if [ -n "${EXTERNALSRC}" ]; then
        ln -sf "${S}/${FPGA_SUBDIRECTORY}/${BITFILE_NAME}.bit" "${UHD_IMAGES_DOWNLOAD_DIR}/${BITFILE_NAME}.bit"
        ln -sf "${S}/${FPGA_SUBDIRECTORY}/${BITFILE_NAME}.dts" "${UHD_IMAGES_DOWNLOAD_DIR}/${BITFILE_NAME}.dts"
    else
...
do_install_append_ni-titanium() {
    ...
    install -m 0644 ${UHD_IMAGES_DOWNLOAD_DIR}/${BITFILE_NAME}.* ${D}/${UHD_IMAGES_INSTALL_PATH}

    mv ${D}/lib/firmware/${BITFILE_NAME}.bin ${D}/lib/firmware/x410.bin
    mv ${D}/lib/firmware/${BITFILE_NAME}.dtbo ${D}/lib/firmware/x410.dtbo
}
